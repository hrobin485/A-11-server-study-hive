...
app.put('/submissions/:id/marks', async (req, res) => {
    try {
        const { id } = req.params;
        const { marks } = req.body; // Expect marks in the request body
        ...
        if (result.modifiedCount === 1) {
            ...
            res.status(404).send({ message: 'Submission not found or marks not updated.' });
        }
    } catch (error) {
        console.error('Error updating marks:', error);
        res.status(500).send({ message: 'Failed to update marks.', error });
    }
});
