...
app.post('/submissions', async (req, res) => {
    try {
      ...
      const result = await submissionsCollection.insertOne(submissions);
      res.status(201).send({ message: 'submissions created successfully!', result });
    } catch (error) {
      console.error('Error creating submissions:', error);
      res.status(500).send({ message: 'Failed to create submissions.', error });
    }
  });
