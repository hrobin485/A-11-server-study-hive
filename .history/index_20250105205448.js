...
app.post('/submissions', async (req, res) => {
    try {
      const submissions = req.body; // Extract the data sent from the frontend
      const result = await submissionsCollection.insertOne(submissions); // Insert into the database
      res.status(201).send({ message: 'Submissions created successfully!', result }); // Send success response
    } catch (error) {
      console.error('Error creating submissions:', error); // Log the error
      res.status(500).send({ message: 'Failed to create submissions.', error }); // Send error response
    }
});
