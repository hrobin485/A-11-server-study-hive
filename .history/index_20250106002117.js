...
app.get('/submissions', async (req, res) => {
  ...
  app.get('/attempted-assignments', async (req, res) => {
    ...
    try {
      const assignments = await submissionsCollection.find({ email }).toArray(); // Find assignments by email
      if (assignments.length > 0) {
        res.status(200).json(assignments); // Send the assignments
      } else {
        res.status(404).send('No attempted assignments found.');
      }
    } catch (error) {
      console.error('Error fetching assignments:', error);
